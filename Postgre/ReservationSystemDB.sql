...
declare
  loc_password record;
begin 
   select into loc_password * from tbllogin where password = par_password;
...
declare
  loc_tablerow record;
begin 
    select into loc_tablerow * from tbllogin where password = par_password;
	
    if par_password = loc_tablerow.password then
		if par_password isnull or par_new_password isnull then
			return json_build_object(
